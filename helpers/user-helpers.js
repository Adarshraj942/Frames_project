...
module.exports={
    ...
    otpUserCreation : (phone) => {
        return new Promise(async(resolve, reject) => {
            ...
            if(user){
                ...
                let data = await db.get().collection(collection.USER_COLLECTION).insertOne(
                    {   
                        phone: phone, 
    ...
    emailchecker:(mail)=>{
        return new Promise(async (res,rej)=>{
         let found=await db.get().collection(collection.USER_COLLECTION).findOne({Email:mail})
            res(found)
        })
    },
       blockUser:(userId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.USER_COLLECTION).updateOne({_id:objectId(userId)},{$unset:{auth:true}}).then((response)=>{
                resolve(response)
            })
        })
    ...
    unblockUser:(userId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.USER_COLLECTION).updateOne({_id:objectId(userId)},{$set:{auth:true}}).then((response)=>{
                resolve(response)
            })
        })
    ...
    showProduct:(proId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.PRODUCT_COLLECTION).find({_id:objectId(proId)}).then((response)=>{
                resolve(response)
            })
        })
    ...
    getUserOrders:(userId)=>{
        return new Promise(async(resolve,reject)=>{
            ...
            .find({userId:objectId(userId)}).sort({$natural:-1}).limit(10).toArray()
            console.log(orders);
    ...
    getUserCount:async ()=>{
        return new Promise((resolve,reject)=>{
            let userCount=db.get().collection(collection.USER_COLLECTION).find().count()
            resolve(userCount)
        })
    }
